...
here we resample the 'emotion_study' dictionaries from 256 Hz to 500 Hz.

Once the dictionaries are resampled, we can compute the BoWav feature using the
...
class Args:
    ...
    path_to_codebooks = root.joinpath("results/emotion_study/dictionaries_resampled")
    path_to_centroid_assignments = root.joinpath("data/cue/centroid_assignments")
    window_length = 1.5
    minutes_per_ic = 50.0
    num_clusters = 128
    centroid_length = 1.0
